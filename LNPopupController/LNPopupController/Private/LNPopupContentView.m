...
- (void)setPopupCloseButtonStyle:(LNPopupCloseButtonStyle)popupCloseButtonStyle
{
	...
	[UIView performWithoutAnimation:^{
		...
		if(buttonStyle != LNPopupCloseButtonStyleNone)
		{
			...
			{
                #if TARGET_OS_MACCATALYST
                _popupCloseButtonLeadingConstraint = [self.popupCloseButton.trailingAnchor constraintEqualToAnchor:self.contentView.trailingAnchor constant:-12];
                #else
...
- (void)_repositionPopupCloseButton
{
	...
	if(possibleBar)
	{
        ...
        if (!_currentPopupContentViewController.popupBar.isInlineWithTabBar) {
            UIView* viewToUse = _currentPopupContentViewController.popupPresentationContainerViewController.presentingViewController.presentedViewController.view;
            if(viewToUse == nil)
            {
                viewToUse = self.superview;
            ...
            #if TARGET_OS_MACCATALYST
            _popupCloseButtonTopConstraint.constant = viewToUse.frame.origin.y + 60;
            #else
            _popupCloseButtonTopConstraint.constant = viewToUse.frame.origin.y + 80;
            #endif
        }
    }
